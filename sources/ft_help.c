...
void	write_command(t_env *env)
{
	...
	mlx_string_put(env->mlx, env->s_win, 0, 230, WHI,
		"-/_ = Diminution de l'iteration");
	mlx_string_put(env->mlx, env->s_win, 0, 295, WHI,
		"R, G ou B = Change la couleur");
	mlx_string_put(env->mlx, env->s_win, 0, 310, WHI,
		"C = Change de couleur aleatoirement");
...
{
	env->s_win = mlx_new_window(env->mlx, 450, 450, "HELP_FRACT_OL");
	write_command(env);
	write_title1(env);
	write_title2(env);
